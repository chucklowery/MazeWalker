...
public class RuthsMazeWanderer2 implements MazeWanderer {
    Grid map = new Grid();
    ...
    public Direction move(Sensor sensor) {
        ...
        if (directions.isEmpty()) {
            ...
            if (destinations.isEmpty()) {
                ...
                Location x = destinations.stream().min((o1, o2) -> (int) -(o1.dist(current) - o2.dist(current)) ).get();
                System.out.println(current);
                System.out.println("GoTo");
                System.out.println(x);
                directions = map.findPath(current, x);
...
//        }
    ...
    static class Grid {
        ...
        HashMap<Location, Tile> graph = new LinkedHashMap<>();
...
//        }
         ...
         LinkedList<Direction> findPath(Location start, Location end) {
            ...
            if(predecessor.get(end) == null) {
                List<Location> passableNeighbors = findPassableNeighbors(end);
                for (Location passableNeighbor : passableNeighbors) {
                    end = passableNeighbor;
                }
        ...
        public List<Direction> findDirectionsToLastPassableTileUnexplored(Location start) {
            ...
            for (int i = topLeftCorner.getX(); i <= bottomRightCorner.getX(); i++) {
                ...
                if (current != null && current.equals(Tile.LAND)) {
                    locations.add(new Location(i, topLeftCorner.getY()));
//                    return findPath(start, new Location(i, topLeftCorner.getY()));
            ...
            for (int j = topLeftCorner.getY(); j >= bottomRightCorner.getY(); j--) {
                ...
                if (current != null && current.equals(Tile.LAND)) {
                    locations.add(new Location(bottomRightCorner.getX(), j));
//                    return findPath(start, new Location(bottomRightCorner.getX(), j));
            ...
            for (int i = topLeftCorner.getX(); i <= bottomRightCorner.getX(); i++) {
                ...
                if (current != null && current.isPassable()) {
                    locations.add(new Location(i, bottomRightCorner.getY()));
//                    return findPath(start, new Location(i, bottomRightCorner.getY()));
            ...
            for (int j = topLeftCorner.getY(); j >= bottomRightCorner.getY(); j--) {
                ...
                if (current != null && current.equals(Tile.LAND)) {
                    locations.add(new Location(topLeftCorner.getX(), j));
//                    return findPath(start, new Location(topLeftCorner.getX(), j));
                }
            }

            if(!locations.isEmpty()){
                Location location = locations.stream().min((o1, o2) -> (int) -(o1.dist(start) - o2.dist(start))).get();
                return findPath(start, location);
            }
            return null;
        }
    ...
    static class Location {
        ...
        private double dist(Location loc) {
